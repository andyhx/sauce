...
int SLBP::blockWidth = 16;
int SLBP::threshold = 30;

auto SLBP::convert_to_grayscale(Acc a) -> Acc {
    Mat gray_image;
...
auto SLBP::threshold_masks(Acc a) -> Acc {
  Mat image = a.m;
...
auto SLBP::perform_binning(Acc a) -> Acc {
    Mat features = a.m;
...
auto SLBP::normalize_blocks(Acc a) -> Acc {
    Mat bins = a.m;
...
auto SLBP::flatten_features(Acc a) -> Acc {
    a.m = a.m.reshape(0, 1);
    return a;
}

vector<function<Acc(Acc)>> SLBP::features() {
